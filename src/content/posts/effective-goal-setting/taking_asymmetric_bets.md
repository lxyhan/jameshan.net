...
The [previous piece](/effective-goal-setting/fat_tails_in_value) established a claim: the top of the distribution is underpriced relative to the middle, because fear, information asymmetry, resource constraints, and social norms push people away from high-value, low-probability goals. This piece is about how to capture that arbitrage.

---

## 1. The Mechanism Is Volume

If the expected value of a high-value, low-probability goal is favorable, the straightforward response is to take the bet repeatedly. Most attempts will fail. This is expected. The math works over volume.
...
This is Taleb's core contribution, and he is correct about it. In a world where value is fat-tailed and the cost of each attempt is bounded, repeated asymmetric bets are positive expected value even when the majority of them fail.

---

## 2. The Constraint Is Opportunity Cost
...
Conversely, some bets look cheap but have hidden costs. Applying to a hundred positions sounds like volume, but if each application requires tailoring that takes significant time, the effective cost per bet is higher than it seems. The volume strategy only works if the per-unit cost is genuinely low.
...
This framework is mechanical. It treats goal selection as a portfolio optimization problem. Given the structure of value, given the cost constraints, here is how to allocate.

What it does not address is why people fail to execute this even when they understand it. The answer, as noted in the previous piece, is primarily psychological. Fear of failure is not an information problem. Knowing the math does not make you less afraid.

That is a different problem, and it is treated [separately](/effective-goal-setting/the_psychology_of_failure).
